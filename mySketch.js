...
const CONFIG = {
  ...
  BLOCK_SIZE: 24,                  // Size of each block in pixels (try: 20-100)
  BORDER_WIDTH: 2,                   // Width of the dark border in blocks (try: 1-3)
  ...
  // Shape settings
  TOTAL_SHAPES: 17,                  // Number of different shapes available (max 30)
  SHAPE_LAYERS: 3,                   // Number of layers per shape pattern (try: 2-6)
  
  // Animation settings
  ANIMATION_SPEED: 2600,              // Animation speed divisor (try: 200-800, larger = slower)
  ...
  FORCE_PALETTE_INDEX: 32,            // Force specific palette (0-17, only if USE_RANDOM_PALETTE is false)
...
function drawShape(d, repeat_num, shape_num) {
  ...
  switch (shape_num) {
    ...
    case shapes_arr[3]: // Hollow square (square with cutout center) - GEOMETRIC
      ...
      break;
    case shapes_arr[4]: // Chevron/arrow pattern (angular pointing shape) - GEOMETRIC
      for (let scl = 1; scl > 0; scl -= 1 / repeat_num) {
        fill(palette[(shape_num+v++)%palette.length]);
        beginShape();
        let size = (d / 2) * scl;
        // Create chevron/arrow pointing right
        vertex(-size, -size/2);
        vertex(0, -size/2);
        vertex(size/2, 0);
        vertex(0, size/2);
    ...
    case shapes_arr[5]: // Hourglass/bowtie (two triangles touching at points) - GEOMETRIC
      ...
      break;
    case shapes_arr[6]: // Lightning bolt (zigzag angular pattern) - ANGULAR
      for (let scl = 1; scl > 0; scl -= 1 / repeat_num) {
        fill(palette[(shape_num+v++)%palette.length]);
        let size = (d / 2) * scl;
        beginShape();
    ...
    case shapes_arr[8]: // Checkerboard pattern (2x2 grid) - GEOMETRIC
      ...
      break;
    case shapes_arr[9]: // Horizontal rectangle (half-width) - RECTANGULAR
      translate(-d / 2, -d / 2);
      rectMode(CORNER);
      for (let scl = 1; scl > 0; scl -= 1 / repeat_num) {
				fill(palette[(shape_num+v++)%palette.length]);
      ...
      break;
    case shapes_arr[10]: // Vertical rectangle (half-height) - RECTANGULAR
      translate(-d / 2, -d / 2);
      rectMode(CORNER);
      for (let scl = 1; scl > 0; scl -= 1 / repeat_num) {
				fill(palette[(shape_num+v++)%palette.length]);
      ...
      break;
    case shapes_arr[11]: // Vertical bar (narrow rectangle) - RECTANGULAR
      push();
      translate(-d / 2, -d / 2);
      rectMode(CORNER);
      for (let scl = 1; scl > 0; scl -= 1 / repeat_num) {
...
function drawFooterText() {
  ...
  const xLeft = offset_w + block_size / 2;
  const yPos = offset_h + block_num_h * block_size - block_size / 2;
  const xRight = offset_w + block_num_w * block_size - block_size / 2;

  fill(0, 0, 100); // White text for contrast
  textSize(block_size * 0.8); // Smaller text to fit in the border
  ...
  text("pirouette v.01", xRight, yPos);
}
